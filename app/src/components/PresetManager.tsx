...
export const PresetManager: React.FC = () => {
  ...
  const savePatch = () => {
    ...
    const moduleStates: Record<string, Record<string, unknown>> = {};
  ...
  const clearPresets = () => {
    if (
      window.confirm(
        "Are you sure you want to delete all saved patches? This cannot be undone.",
      )
    ) {
  ...
  const exportPatch = () => {
    ...
    const blob = new Blob([JSON.stringify(patchData, null, 2)], {
  ...
  const importPatch = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        console.error(err);
        alert(
          "The file failed to load, please try again. If the problem persists, check for issues with the patch data.",
        );
      }
    };
    ...
    <div className="flex gap-2 items-center">
      <DropdownMenu>
        ...
        <DropdownMenuContent className="bg-zinc-950 border-zinc-800 text-zinc-100 w-56">
          ...
          {presets.length === 0 ? (
            <div className="p-2 text-sm text-zinc-500 italic">
              No saved patches
            </div>
          ) : (
            <>
      ...
      <div className="relative">
        ...
        <Button
          variant="outline"
          size="sm"
          className="gap-2 bg-zinc-900 border-zinc-800 hover:bg-zinc-800 text-zinc-300 pointer-events-none"
        >
